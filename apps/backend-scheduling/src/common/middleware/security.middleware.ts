...
export function tenantRateLimitKeyGenerator(
  req: Request & { context?: { organizationId?: string } },
): string {
  // Use organization ID if available, otherwise fall back to IP
  const organizationId = req.context?.organizationId || (req as any).get?.('x-organization-id');
  return organizationId || (req as any).ip || 'unknown';
}
