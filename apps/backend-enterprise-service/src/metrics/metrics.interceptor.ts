...
export class MetricsInterceptor implements NestInterceptor {
  // Logger instance not currently used but kept for future debugging
  // private readonly _logger = new Logger(MetricsInterceptor.name);
  ...
  private getRoutePattern(context: ExecutionContext): string {
    // Handler and controller metadata not currently used
    const request = context.switchToHttp().getRequest();
