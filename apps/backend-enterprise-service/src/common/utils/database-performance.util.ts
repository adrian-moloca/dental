...
export async function analyzeQueryPerformance<T>(
  ...
  const explain: any = await query.explain('executionStats');
...
export async function trackQueryPerformance<T>(
  ...
  // Execute query
  const result = (await query.exec()) as any as T;

  // Get explain for metrics
  const explain: any = await queryFn().explain('executionStats');
  const stats = (explain as any).executionStats as any;
...
export async function cursorPaginate<T>(
  ...
  if (cursor) {
    ...
    const lastDoc: any = await model.findById(lastId).select(sortField).lean();

    if (lastDoc && lastDoc[sortField]) {
      query = query.where(sortField)[sortOrder === 'desc' ? 'lt' : 'gt'](lastDoc[sortField]);
    }
  }
  ...
  const results = (await query.exec()) as any[];
