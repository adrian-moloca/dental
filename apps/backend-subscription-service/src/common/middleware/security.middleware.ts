 ...
 *
 * @param req - Express request with optional context
 * @returns Rate limit key
 */
export function tenantRateLimitKeyGenerator(
  req: (Request & { context?: { organizationId?: string } }) | Request,
): string {
  // Use organization ID if available, otherwise fall back to IP
  const customReq = req as Request & { context?: { organizationId?: string } };
  const organizationId = customReq.context?.organizationId || customReq.get?.('x-organization-id');
  const ipAddress = (customReq.ip || customReq.socket?.remoteAddress) as string;
  return organizationId || ipAddress || 'unknown';
}
