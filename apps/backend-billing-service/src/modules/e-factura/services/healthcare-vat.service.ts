...
export class HealthcareVatService {
   ...
   */
  getVatTreatment(procedureCode?: string, itemType?: string, isCosmetic?: boolean): VatTreatment {
    ...
    if (!procedureCode) {
      this.logger.debug('No procedure code provided, using default medical VAT treatment');
  ...
  private getCategoryFromCode(procedureCode: string): DentalProcedureCategory {
    ...
    this.logger.debug(`Unknown procedure code prefix: ${prefix}, defaulting to MEDICAL`);
