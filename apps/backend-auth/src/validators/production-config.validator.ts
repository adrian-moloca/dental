 ...
 * - JWT secrets must be strong and unique (minimum 32 characters)
...
export function validateProductionConfig(config: ConfigService<any, boolean>): void {
  ...
  // CRITICAL: JWT refresh secret must be strong
  const jwtRefreshSecret = config.get<string>('jwt.refreshSecret');
  if (!jwtRefreshSecret || jwtRefreshSecret.length < 32) {
    throw new Error(
      'CRITICAL SECURITY ERROR: JWT refresh secret is too weak. ' +
        'Must be at least 32 characters in production. ' +
        'Use a cryptographically random string (e.g., openssl rand -base64 32).'
    );
  }
  ...
  // Check for common weak secrets in JWT refresh secret
  const refreshSecretLower = jwtRefreshSecret.toLowerCase();
  for (const pattern of weakPatterns) {
    if (refreshSecretLower.includes(pattern)) {
      throw new Error(
        `CRITICAL SECURITY ERROR: JWT refresh secret contains '${pattern}' and appears to be a development/test value. ` +
          'Use a strong, unique secret in production. ' +
          'Generate with: openssl rand -base64 32'
      );
    }
  ...
  // CRITICAL: Access and refresh secrets must be different
  if (jwtAccessSecret === jwtRefreshSecret) {
    throw new Error(
      'CRITICAL SECURITY ERROR: JWT access and refresh secrets must be different. ' +
        'Using the same secret for both tokens reduces security. ' +
        'Generate two separate secrets with: openssl rand -base64 32'
    );
  }
