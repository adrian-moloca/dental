 ...
 * Adapted for Auth Service (PostgreSQL-based)
 */
...
export class CacheService implements OnModuleDestroy {
  ...
  private readonly TTL_STRATEGIES = {
    ...
    // MFA tokens (very short-lived)
    mfaToken: 300, // 5 minutes

    // Device fingerprints
    device: 1800, // 30 minutes
  ...
  /**
   * Invalidation helpers for Auth Service
   */
  ...
  async invalidateUserSessions(userId: string): Promise<void> {
    await this.delPattern(`session:*:user:${userId}`);
  }

  // Invalidate tenant cache
  async invalidateTenant(tenantId: string): Promise<void> {
    await this.del([`tenant:${tenantId}`, `tenant:${tenantId}:*`]);
  }
