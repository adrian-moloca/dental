...
export class HealthService extends HealthIndicator {
  ...
  constructor(private readonly subscriptionClient: SubscriptionClientService) {
    super();
  }
  ...
  async checkRedis(key: string): Promise<HealthIndicatorResult> {
    try {
      // TODO: Implement Redis ping when Redis client is added in AUTH-002
  ...
  /**
   * Check subscription service connectivity and circuit breaker state
   *
   * Validates that the subscription service dependency is healthy.
   * Checks circuit breaker state to ensure auth service can communicate with subscription service.
   *
   * @param key - Health check key
   * @returns Health indicator result
   */
  async checkSubscriptionService(key: string): Promise<HealthIndicatorResult> {
    try {
      // Get circuit breaker stats
      const cbStats = this.subscriptionClient.getCircuitBreakerState();

      // Circuit breaker OPEN means service is down
      const isHealthy = cbStats.state !== 'OPEN';

      const result = this.getStatus(key, isHealthy, {
        state: cbStats.state,
        failureCount: cbStats.failureCount,
        successCount: cbStats.successCount,
        nextAttempt: cbStats.nextAttempt.toISOString(),
      });
      ...
      throw new HealthCheckError('Subscription service circuit breaker is OPEN', result);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(`Subscription service health check failed: ${errorMessage}`);

      if (error instanceof HealthCheckError) {
        throw error;
      }

      throw new HealthCheckError('Subscription service health check failed', {
        [key]: {
          status: 'down',
          message: errorMessage,
        },
